class YHPerk_Demolitionist extends KFPerk_Demolitionist;
...
static function bool IsDamageTypeOnPerk( class<KFDamageType> KFDT )
{
    if( KFDT != none )
    {
        return KFDT.default.ModifierPerkList.Find( class'KFPerk_Demolitionist' ) > INDEX_NONE;
    }
...
{
    if ( InstigatorPerkClass == class'YHPerk_Demolitionist' )
    {
        InstigatorPerkClass = class'KFPerk_Demolitionist';
    }

    if( W != none )
    {
        return W.static.GetWeaponPerkClass( InstigatorPerkClass ) == class'KFPerk_Demolitionist';
    }
    else if( WeaponPerkClass.length > 0 )
    {
        return WeaponPerkClass.Find(class'KFPerk_Demolitionist') != INDEX_NONE;
    }
...
event NotifyPerkModified()
{
    ...
    ScriptTrace();
    `log("CALLING MODIFIED FROM Demolitionist");
