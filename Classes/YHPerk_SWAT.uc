...
static function bool IsDamageTypeOnPerk( class<KFDamageType> KFDT )
{
    if( KFDT != none )
    {
        return KFDT.default.ModifierPerkList.Find( class'KFPerk_SWAT' ) > INDEX_NONE;
    }
                ...
                )
{
    ...
    {
        InstigatorPerkClass = class'KFPerk_SWAT';
    }

    if( W != none )
    {
        return W.static.GetWeaponPerkClass( InstigatorPerkClass ) == class'KFPerk_SWAT';
    }
    else if( WeaponPerkClass.length > 0 )
    {
        return WeaponPerkClass.Find(class'KFPerk_SWAT') != INDEX_NONE;
    }
...
event NotifyPerkModified()
{
    ...
    ScriptTrace();
    `log("CALLING MODIFIED FROM SWAT");
